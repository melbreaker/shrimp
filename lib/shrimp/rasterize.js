...
if (system.args.length < 3 || system.args.length > 13) {
  ...
  footer = {height: "2cm", contents: phantom.callback(function(pageNum, numPages) {return pageNum == numPages ? '<div style="background:#d8474c;color:white;padding:10px;text-align:center;width:100%;">USAY.co</div>' : '' ;})}
  if (system.args.length > 3 && system.args[2].substr(-4) === ".pdf") {
    size = system.args[3].split('*');
    page_options.paperSize = size.length === 2 ? { width:size[0], height:size[1], margin:'0px', footer: footer }
        : { format:system.args[3], orientation:orientation, margin:margin, footer: footer };
  }
